defmodule Circus do
  ...
  Some programs, each with a name & weight are stacked
  in an acyclic directed graph on top of one another.

  Given a list of labels, weights, and their children like:
  ...
  we can reconstruct the graph to look like:
  ...
  Given the graph, we need to:

  1. Determine which node is the root node.
  2. What should the unbalanced program weigh to balance the whole graph?
  """
